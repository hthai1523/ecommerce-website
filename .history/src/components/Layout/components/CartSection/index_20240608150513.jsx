...
function CartSection({ data }) {
  ...
  const handleMinusQuantity = (data) => {
    dispatch(handleMinusQuantity(data.id));
  };

  const handlePlusQuantity = (data) => {
    dispatch(handlePlusQuantity(data.id));
  };
  ...
  return (
    <tbody className="">
      <tr>
        ...
        <td className="text-right w-[8%] mt-9">{formatCurrentcy(price)}</td>
      </tr>
    </tbody>
  );
}
