...
function CommentSection({ rating, comment, date, reviewerName }) {
        ...
        <div className="flex-1 space-y-2">
          ...
          <div className="flex items-center gap-4">
            <Button onClick={handleLike} className="text-base font-medium w-10 transition-all duration-300">
              {isLike ? <AiFillLike size={20} /> : "Like"}
            </Button>
            <Button onClick={handleReply} className="text-base font-medium">Reply</Button>
            <p>{formattedDate}</p>
          </div>
        </div>
        <div className="flex gap-1 items-center absolute top-4 right-4">
          {rating && [...Array(Math.floor(rating))].map((_, index) => (
            <FaStar key={index} className="text-yellow-400" />
          ))}
        </div>
      </div>
      {isReply && <div className="ml-10"><CommentForm /></div>}
    </>
  );
}
