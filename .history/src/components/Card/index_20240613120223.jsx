...
function Card({ data, url }) {
  ...
  return (
    <div className="relative flex flex-col shadow-lg  rounded-2xl cursor-pointer p-4 mx-auto lg:w-full md:w-full sm:w-[90%] hover:scale-105 duration-300">
      {data.rating > 4.5 && (
        <span className="absolute top-0 left-0 bg-red-500 text-white text-xs font-semibold p-2 rounded-tl-md">
          Best Seller
        </span>
      )}
      <Link to={routes.productDetail.replace(":id", data.id)}>
        <Image
          src={data.thumbnail}
          alt={data.title}
      ...
      <div className="w-full flex-1 text-center mt-4 text-black">
        ...
        <Button
          ...
          className="text-center font-medium text-sm lg:text-base cursor-pointer border-2 border-solid rounded-lg border-primary text-white py-2 lg:py-3 px-4 lg:px-12 mt-4 bg-primary hover:opacity-70 transition-opacity duration-150 ease-in-out"
        >
          ADD TO CART
        </Button>
      </div>
