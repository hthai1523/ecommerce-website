...
function Card({ data, url, isBestSeller = true }) {
  ...
  return (
    <div className="flex flex-col gap-2 raltive shadow-lg  bg-white  rounded-2xl cursor-pointer  p-2 my-5 mx-auto w-[215px] lg:w-[350px] hover:scale-105 duration-300">
      ...
      <div className="w-full mx-auto text-center mx-auto mt-4 text-black ">
        <h4 className="font-bold text-2xl truncate">{data.title}</h4>
        <div className="flex flex-row items-center tece">
          <p className="font-normal text-xl text-primary">{formatCurrentcy(realPrice)}</p>
          <p className="font-normal text-base line-through">{formatCurrentcy(data.price)}</p>
        </div>
        <Button
          onClick={(e) => {
            handleAddToCart(e);
