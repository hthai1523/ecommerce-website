          ...
          {product?.images.map((image, index) => {
            return (
              <Image
                ref={imageRef}
                key={index}
                className={`w-full h-full block object-cover transition-all duration-1000 ease-in-out  `}
                style={{translate: `${-100 * imageIndex}%`, flexShrink: '0', flexGrow: '0',}}
                src={image}
              />
            );
          })}
          <div onClick={handlePrevImage} className="absolute top-[50%] left-[0] translate-y-[-50%]  text-2xl text-black font-black bg-[#EEE] rounded-full w-fit h-fit my-auto p-2 cursor-pointer hover:scale-110 duration-150 ">
            <FaChevronLeft />
          </div>
          <div onClick={handleNextImage} className="absolute top-[50%] right-[0] translate-y-[-50%]  text-2xl text-black font-black bg-[#EEE] rounded-full w-fit h-fit my-auto p-2 cursor-pointer hover:scale-110 duration-150 ">
            <FaChevronRight />
          </div>
        </div>
